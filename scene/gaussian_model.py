...
class GaussianModel:
    ...
    def get_xyz(self):
        ...
        if self.rotate_xyz:
            ...
            for pid in range(1, num_parts):
                if self.rot_type == "6d":
                    ...
                    R = quaternion_to_matrix(
                        self.rotation_activation(self.R[pid - 1][None])
                    )[0]
                else:
                    R = self.R[pid - 1]
                try:
            ...
            if self.rot_type == "6d":
                ...
                R = quaternion_to_matrix(self.rotation_activation(self.current_R))
            else:
                R = self.current_R
    ...
    def get_RT(self, idx=None):
        ...
        if self.rot_type == "6d":
            ...
            R = quaternion_to_matrix(self.rotation_activation(self.R[idx][None]))[0]
        else:
            R = self.R[idx]
        ...
        Rt = torch.cat(
            [Rt, torch.tensor([[0, 0, 0, 1]], dtype=Rt.dtype, device=Rt.device)], dim=0
        )
        return Rt
    ...
    def fix_position(self):
        ...
        self._scaling = nn.Parameter(self._scaling.detach().requires_grad_(False))
        self._rotation = nn.Parameter(self._rotation.detach().requires_grad_(False))
        self._opacity = nn.Parameter(self._opacity.detach().requires_grad_(False))
        _xyz = self._xyz.detach().clone()

    def training_setup(
        self, training_args, fix_pos=False, fix_feat=False, fit_pose=False
    ):
        ...
        l += [
            {"params": [self._xyz], "lr": _xyz_lr, "name": "xyz"},
            {
                ...
                "lr": training_args.feature_lr * feat_lr_factor,
                "name": "f_dc",
            },
            {
                "params": [self._features_rest],
                "lr": training_args.feature_lr / 20.0 * feat_lr_factor,
                "name": "f_rest",
            },
            {
                "params": [self._opacity],
                "lr": training_args.opacity_lr * feat_lr_factor,
                "name": "opacity",
            },
            {
                "params": [self._scaling],
                "lr": training_args.scaling_lr * feat_lr_factor,
                "name": "scaling",
            },
            {
        ...
        if fit_pose:
            rotation_lr_factor = 0.1 if (not fix_pos) or (not fix_feat) else 1.0
            if self.rotate_seq:
    ...
    def init_RT(self, pcd=None, pose=None):
        ...
        if self.rot_type == "axis":
            ...
            if pose is not None:
                ...
                R_init = torch.tensor([1, 0, 0, 0], dtype=torch.float32, device="cuda")
            R = torch.stack([R_init for _ in range(1, num_parts)])
        else:
            if pose is not None:
                R_init = pose[:3, :3]
    ...
    def init_RT_seq(self, seq_len, pose=None):
        if pose is None:
            ...
            if self.rot_type == "axis":
                ...
                R = torch.tensor([1, 0, 0, 0], dtype=torch.float32, device="cuda")
            else:
                R = torch.eye(3, device="cuda")

            T = torch.zeros(1, 3, device="cuda", dtype=torch.float32)
            self.R = [nn.Parameter(R.requires_grad_(True)) for _ in range(seq_len)]
            self.T = [nn.Parameter(T.requires_grad_(True)) for _ in range(seq_len)]
        else:
            ...
            self.R = [nn.Parameter(R[i].requires_grad_(True)) for i in range(seq_len)]
            self.T = [nn.Parameter(T[i].requires_grad_(True)) for i in range(seq_len)]
    ...
    def load_ply(self, path, num_gauss=-1):
        ...
        self._xyz = nn.Parameter(
            torch.tensor(
                xyz[:num_gauss], dtype=torch.float, device="cuda"
            ).requires_grad_(True)
        )
        self._features_dc = nn.Parameter(
            torch.tensor(features_dc[:num_gauss], dtype=torch.float, device="cuda")
            .transpose(1, 2)
            .contiguous()
            .requires_grad_(True)
        )
        self._features_rest = nn.Parameter(
            torch.tensor(features_extra[:num_gauss], dtype=torch.float, device="cuda")
            .transpose(1, 2)
            .contiguous()
            .requires_grad_(True)
        )
        self._opacity = nn.Parameter(
            torch.tensor(
                opacities[:num_gauss], dtype=torch.float, device="cuda"
            ).requires_grad_(True)
        )
        self._scaling = nn.Parameter(
            torch.tensor(
                scales[:num_gauss], dtype=torch.float, device="cuda"
            ).requires_grad_(True)
        )
        self._rotation = nn.Parameter(
            torch.tensor(
                rots[:num_gauss], dtype=torch.float, device="cuda"
            ).requires_grad_(True)
        )
...
class GS_Render:
    def __init__(
        ...
        self.gaussians = GaussianModel(
            sh_degree, rot_type=self.rot_type, view_dependent=self.view_dependent
        )
        ...
        if colors_precomp is None:
            if convert_SHs_python:
                if self.view_dependent:
                    ...
                    camera_center = (
                        self.gaussians.get_RT(fidx).inverse()[:3, 3].detach()
                    )
