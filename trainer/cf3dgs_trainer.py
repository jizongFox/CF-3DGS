...
class CFGaussianTrainer(GaussianTrainer):
    def __init__(self, data_root, model_cfg, pipe_cfg, optim_cfg):
        ...
        self.gs_render_local = GS_Render(
            white_background=False,
            view_dependent=model_cfg.view_dependent,
        )
    ...
    def train_step(
        ...
        **kwargs,
        ...
        if prev_gaussians is not None:
            ...
            render_pkg["image"] = render_pkg["image"] * mask + render_pkg_prev[
                "image"
            ] * (1 - mask)
            render_pkg["depth"] = render_pkg["depth"] * mask + render_pkg_prev[
                "depth"
            ] * (1 - mask)
        ...
        with torch.no_grad():
            ...
            if iteration < optim_opt.densify_until_iter and densify:
                ...
                try:
                    gs_render.gaussians.max_radii2D[visibility_filter] = torch.max(
                        gs_render.gaussians.max_radii2D[visibility_filter],
                ...
                if (
                    ...
                    self.gs_render.gaussians.densify_and_prune(
                ...
                if (
                    iteration % optim_opt.opacity_reset_interval == 0
                    and reset
                    and iteration < optim_opt.reset_until_iter
                ):
            ...
            if (
                getattr(gs_render.gaussians, "camera_optimizer", None) is not None
                and update_cam
            ):
    ...
    def init_two_view(self, view_idx_1, view_idx_2, pipe, optim_opt):
        ...
        cam_info, pcd, viewpoint_cam = self.prepare_data(
            view_idx_1, orthogonal=True, down_sample=True
        )
        radius = np.linalg.norm(pcd.points, axis=1).max()

        # Initialize gaussians
        self.gs_render.reset_model()
        self.gs_render.init_model(
            pcd,
        )
    ...
    def add_view_v2(self, view_idx, view_idx_prev, reverse=False):
        ...
        # prepare data
        cam_info, pcd, viewpoint_cam = self.prepare_data(
            view_idx_prev, orthogonal=True, down_sample=True
        )
        radius = np.linalg.norm(pcd.points, axis=1).max()
        ...
        for iteration in range(1, optim_opt.iterations + 1):
            # Update learning rate
            self.gs_render_local.gaussians.update_learning_rate(iteration)
            loss, rend_dict_ref, psnr_train = self.train_step(
                self.gs_render_local,
                viewpoint_cam_ref,
                iteration,
                pipe,
        ...
        if self.just_reset:
            ...
            for iteration in range(1, num_iterations):
                ...
                self.gs_render.gaussians.update_learning_rate(self.global_iteration)
                viewpoint_cam = self.load_viewpoint_cam(
                    fidx,
                    pose=self.gs_render.gaussians.get_RT(fidx).detach().cpu(),
                    load_depth=True,
                )
                loss, rend_dict_ref, psnr_train = self.train_step(
                    self.gs_render,
        ...
        if max(view_idx, view_idx_prev) > min(
            int(self.seq_len * 0.8), self.seq_len - 5
        ):
            num_iterations = 1000
    ...
    def create_pcd_from_render(self, render_dict, viewpoint_cam):
        ...
        pts = depth_to_3d(depth[None, None], intrinsics[None], normalize_points=False)
    ...
    def train_from_progressive(
        ...
        for epoch in range(num_eppch):
            ...
            for fidx in range(start_frame, end_frame):
                ...
                render_dict = self.gs_render.render(
                    viewpoint_cam,
                    compute_cov3D_python=pipe.compute_cov3D_python,
                    convert_SHs_python=pipe.convert_SHs_python,
                )
                ...
                print(
                    "Frames {:03d}/{:03d}, PSNR : {:.03f}".format(
                        fidx, self.seq_len - 1, psnr_train
                    )
                )
                self.visualize(
                    render_dict,
            ...
            with torch.no_grad():
                ...
                for val_idx in range(end_frame):
                    viewpoint_cam = self.load_viewpoint_cam(
                        val_idx,
                        pose=self.gs_render.gaussians.get_RT(val_idx).detach().cpu(),
                    )
                    render_dict = self.gs_render.render(
                        viewpoint_cam,
                        compute_cov3D_python=pipe.compute_cov3D_python,
                ...
                print(
                    "Number of {:03d} to {:03d} frames: PSNR : {:.03f}".format(
                        start_frame, end_frame, psnr_test / (end_frame)
                    )
                )
            ...
            torch.save(pose_dict, f"{result_path}/pose/ep{epoch:02d}_init.pth")
            os.makedirs(f"{result_path}/chkpnt", exist_ok=True)
            torch.save(
                self.gs_render.gaussians.capture(),
    ...
    def eval_nvs(
        ...
        result_path = os.path.dirname(self.model_cfg.model_path).replace(
            "chkpnt", "test"
        )
        os.makedirs(result_path, exist_ok=True)
        ...
        pose_test_init = pose_dict_train["poses_pred"][
            int(sample_rate / 2) :: sample_rate - 1
        ][:max_frame]
        self.gs_render.gaussians.init_RT_seq(self.seq_len, pose_test_init.float())
        self.gs_render.gaussians.rotate_seq = True
        self.gs_render.gaussians.training_setup(
            optim_opt,
            fix_pos=True,
        ...
        for epoch in range(num_epochs):
            for fidx in range(self.seq_len):
                ...
                #     fidx, iteration)
                loss_dict, rend_dict, psnr_train = self.train_step(
                    self.gs_render,
                    viewpoint_cam,
                    iteration,
                    pipe,
        ...
        with torch.no_grad():
            for fidx in range(self.seq_len):
                ...
                render_dict = self.gs_render.render(
                    viewpoint_cam, compute_cov3D_python=False, convert_SHs_python=False
                )
                gt_image = viewpoint_cam.original_image.cuda()
                psnr_test += psnr(render_dict["image"], gt_image).mean().double()
                ssim_test += ssim(render_dict["image"], gt_image).mean().double()
                lpips_test += (
                    lpips(render_dict["image"], gt_image, net_type="vgg")
                    .mean()
                    .double()
                )
                self.visualize(
        ...
        with open(f"{result_path}/test.txt", "w") as f:
            f.write(
                "PSNR : {:.03f}, SSIM : {:.03f}, LPIPS : {:.03f}".format(
                    psnr_test / end_frame, ssim_test / end_frame, lpips_test / end_frame
                )
            )
            f.close()

        print(
            "Number of {:03d} to {:03d} frames: PSNR : {:.03f}, SSIM : {:.03f}, LPIPS : {:.03f}".format(
                start_frame,
                end_frame,
                psnr_test / end_frame,
                ssim_test / end_frame,
    ...
    def eval_pose(
        ...
        trans_gt_align, trans_est_align, _ = self.align_pose(
            poses_gt[:, :3, -1].numpy(), poses_pred[:, :3, -1].numpy()
        )
        ...
        with open(f"{result_path}/pose_eval.txt", "w") as f:
            f.write(
                "RPE_trans: {:.03f}, RPE_rot: {:.03f}, ATE: {:.03f}".format(
    ...
    def render_nvs(self, traj_opt="bspline", N_novel_imgs=120, degree=100):
        ...
        if traj_opt == "bspline":
            ...
            c2ws = interp_poses_bspline(
                poses_pred_w2c_train.inverse(), N_novel_imgs, i_train, degree
            )
            w2cs = c2ws.inverse()
        ...
        for fidx, pose in enumerate(w2cs):
            ...
            self.visualize(
                render_dict, f"{render_dir}/img_out/{fidx:04d}.png", save_ply=False
            )
        ...
        imageio.mimwrite(
            os.path.join(video_out_dir, f"{self.category}_{self.seq_name}_ours.mp4"),
            imgs,
            fps=30,
            quality=9,
        )
    ...
    def visualize(
        self, render_pkg, filename, gt_image=None, gt_depth=None, save_ply=False
    ):
        ...
        if "depth" in render_pkg:
            ...
            if gt_depth is not None:
                gt_depth = Image.fromarray(
                    colorize(gt_depth.detach().cpu().numpy(), cmap="magma_r")
                ).convert("RGB")
                rend_depth = Image.fromarray(
                    np.hstack([np.asarray(gt_depth), np.asarray(rend_depth)])
                )
        ...
        rend_img = Image.fromarray(
            np.asarray(
                render_pkg["image"].detach().cpu().permute(1, 2, 0).numpy() * 255.0,
                dtype=np.uint8,
            )
        ).convert("RGB")
        if gt_image is not None:
            gt_image = Image.fromarray(
                np.asarray(
                    gt_image.permute(1, 2, 0).cpu().numpy() * 255.0, dtype=np.uint8
                )
            ).convert("RGB")
            rend_img = Image.fromarray(
                np.hstack([np.asarray(gt_image), np.asarray(rend_img)])
            )
    ...
    def construct_point(self, gs_model, poses, iteration, result_path, stop_frame=-1):
        ...
        with torch.no_grad():
            progress_bar = tqdm(range(self.seq_len), desc="Reconstructing point cloud")
